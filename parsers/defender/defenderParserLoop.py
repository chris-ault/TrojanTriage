#
# Usage: python defenderParserLoop.py C:\directoryOfTXTFiles\
#
# TODO:
...
def parseUpload(defenderOutput):
    inserts = 0
    entry = 0
    fileLocations = ''
                    ...
                    if parts[0].strip() == "ThreatName":
                        ...
                        for single in locationsGroup:
                            ...
                            while len(single.rsplit('_', containerCheck)[1]) < 32:
                                ...
                                if containerCheck > 5:
                                    break
                            testHash = single.rsplit('are_', containerCheck)[1].split("-")[0].strip("}").strip("...")
                        ...
                        for i in range(len(fileHashes)):
                            if checkHash(fileHashes[i]):
                                ...
                                badFiles.append(fileHashes[i])
                        fileLocations = ''
                        locationsGroup = []
                        del fileHashes[:]
                        del fileLocGroup[:]
    print "Bad files not added " + str(badFiles)
    print(inserts, " hash record updated or inserted.")
    mydb.commit()
    fp.close()
...
for (dirpath, dirnames, filenames) in walk(mypath):
    ...
    break
for fileIn in f:
    print "Starting processing of :" + fileIn
    parseUpload(fileIn)
print "all done"
